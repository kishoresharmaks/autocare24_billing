import type { InvoiceItemInput, InvoiceMode, TaxScope } from "../types/cloud";
...
const allocateProportionalCents = (amountCents: bigint, weights: bigint[]) => {
  ...
  const allocations = normalizedWeights.map((weight, index) => ({
    index,
    base: (normalizedAmount * weight) / totalWeight,
    remainder: (normalizedAmount * weight) % totalWeight,
    weight
  }));
